cat > /root/ultimate_nodesingbox_2026.sh << 'ULTIMATE_EOF'
#!/bin/bash
# =====================================================================
# nodesingbox_2026.sh — 用 sing-box 单内核完全替代 xray 的所有节点
# 基于 node6.sh 重构：停用 xray(xbz)，全部入站由 sing-box(bz) 承载。
# xray 旧节点并入 sing-box：XHTTP-CDN(2083)、XHTTP-Reality(2053)、WS直连(80)
# 版本：0.0.1
# =====================================================================
...
echo "================================================================="
echo "      ✨ nodesingbox 0.0.1：sing-box 单内核替代 xray"
echo "            停止旧程序（如果有）"
...
get_zone_id() {
  ...
  if [ -z "$zid" ]; then echo "❌ 获取 ZONE_ID 失败" >&2; return 1; fi
  echo "$zid"
}
...
echo "      ✨ 5. 配置与服务初始化"
mkdir -p "$CERT_DIR"
echo "      ✨ 5.1 写入 sing-box 单内核配置文件（承载全部节点，含原 xray 三节点）"
cat > "$SBOX_CONF" << CONFIG
{
  ...
  "inbounds": [
    ...
    { "type": "vless", "tag": "vless-ws-80-ml", "listen": "::", "listen_port": 80, "users": [{ "uuid": "$MY_GUID" }], "transport": { "type": "ws", "path": "/videos", "headers": { "Host": "$ML_HOST" } } },
    { "type": "vless", "tag": "vless-xhttp-cdn", "listen": "::", "listen_port": $PORT_CDN_VX, "users": [{ "uuid": "$MY_GUID" }], "tls": { "enabled": true, "server_name": "$VX_DOMAIN", "certificate_path": "$CERT_DIR/server.crt", "key_path": "$CERT_DIR/server.key" }, "transport": { "type": "xhttp", "path": "/videos", "mode": "auto" } },
    { "type": "vless", "tag": "vless-xhttp-reality", "listen": "::", "listen_port": $PORT_XHTTP_REALITY, "users": [{ "uuid": "$MY_GUID" }], "tls": { "enabled": true, "server_name": "$DEST_DOMAIN", "reality": { "enabled": true, "handshake": { "server": "$DEST_DOMAIN", "server_port": 443 }, "private_key": "$PRIVATE_KEY", "short_id": ["$SHORT_ID"] } }, "transport": { "type": "xhttp", "path": "/videos", "mode": "auto" } }
  ],
  "outbounds": [{ "type": "direct" }]
}
CONFIG
echo "      ✅ sing-box 单内核配置文件已写入"

echo "      ✨ 5.2 创建 Systemd 服务文件（仅 bz，停用 xbz）"
systemctl disable xbz 2>/dev/null
cat > /etc/systemd/system/bz.service << SVC
[Unit]
Description=bz (sing-box) service
...
issue_cert_try() {
    ...
    if [ "$server" = "zerossl" ]; then
        if [ -z "$ZEROSSL_EAB_KID" ] || [ -z "$ZEROSSL_EAB_HMAC" ]; then return 1; fi
    ...
    if [ "$server" = "google" ]; then
        if [ -z "$GTS_EAB_KID" ] || [ -z "$GTS_EAB_HMAC" ]; then return 1; fi
...
if [ -s "$CERT_DIR/server.crt" ] && [ -s "$CERT_DIR/server.key" ] && cert_matches_domain "$CERT_DIR/server.crt" && ! cert_expired "$CERT_DIR/server.crt"; then
    ...
    if [ -s "$HOME/.acme.sh/${DOMAIN}_ecc/fullchain.cer" ]; then
        "$HOME/.acme.sh/acme.sh" --install-cert -d "$DOMAIN" --ecc --key-file "$CERT_DIR/server.key" --fullchain-file "$CERT_DIR/server.crt" --reloadcmd "systemctl restart bz"
        if [ -s "$CERT_DIR/server.crt" ] && [ -s "$CERT_DIR/server.key" ] && cert_matches_domain "$CERT_DIR/server.crt"; then CERT_OK=1; fi
    else
        echo "      ❌ 证书申请失败，未生成 fullchain.cer，跳过安装"
    fi
fi
[ "${CERT_OK:-0}" -eq 1 ] && echo "      ✅ 通配符证书已就绪" || echo "      ❌ 通配符证书未就绪"

systemctl enable bz && systemctl restart bz
echo "      ✅ sing-box 服务已启动并设置开机自启（xray 已停用）"

echo "      ✨ 6. 开机自动运行一次、每天定时更新"
cat > /etc/systemd/system/ultimate-nodesingbox-boot.service << 'SVC'
[Unit]
Description=Run ultimate_nodesingbox_2026.sh at boot
After=network-online.target
Wants=network-online.target
[Service]
Type=oneshot
ExecStart=/bin/bash /root/ultimate_nodesingbox_2026.sh
RemainAfterExit=yes
[Install]
WantedBy=multi-user.target
SVC
(crontab -l 2>/dev/null | grep -vE "acme.sh|bz-update|xbz-update"
 ...
 echo "0 21 * * * TZ=UTC $BZ_UPD_SH >/dev/null 2>&1") | crontab -
echo "      ✅ 定时任务已设置（每日自动续签证书及更新 sing-box 内核）"
...
echo ""
echo "✅ ✅ ✅ sing-box 单内核部署完成，xray 已完全替代 ✅ ✅ ✅ "
ULTIMATE_EOF
bash /root/ultimate_nodesingbox_2026.sh
systemctl daemon-reload
systemctl enable ultimate-nodesingbox-boot.service
systemctl status ultimate-nodesingbox-boot.service