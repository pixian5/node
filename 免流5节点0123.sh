echo "免流原理：已成功的方法之一是搭建如vless，端口选80，传输层协议选websocket，host填能免流的域名，如v9-y.douyinvod.com"
echo "singbox负责VLESS+TCP+Reality（XTLS-Vision）、Hysteria2。"
echo "xray负责VLESS+XHTTP+TLS（CDN）、VLESS+XHTTP+Reality。"
echo "CDN节点可以优选ip，如xbz.cf.cname.vvhan.com"
#clear
...
cat > "$SBOX_CONF" << CONFIG
{
  ...
  "inbounds": [
    ...
    { "type": "vless", "tag": "vless-ws-ml", "listen": "::", "listen_port": 80, "users": [{ "uuid": "$MY_GUID" }], "transport": { "type": "ws", "path": "/videos", "headers": { "Host": "$ML_HOST" } } }
  ],
    ...
    "inbounds": [
        {
            ...
            "streamSettings": { "network": "xhttp", "security": "tls", "tlsSettings": { "certificates": [{ "certificateFile": "$CERT_DIR/server.crt", "keyFile": "$CERT_DIR/server.key" }], "serverName": "$VX_DOMAIN" }, "xhttpSettings": { "path": "/videos" } }
        },
        {
...
echo ""
echo "- {name: \"免流VLESS80-${MY_SUB}\", type: vless, server: $SUB_DOMAIN, port: 80, uuid: $MY_GUID, network: ws, ws-opts: {path: /videos, headers: {Host: $ML_HOST}}}"
...
echo "- {name: \"Reality-XTLS-Vision443-${MY_SUB}\", type: vless, server: $SUB_DOMAIN, port: 443, uuid: $MY_GUID, network: tcp, tls: true, flow: xtls-rprx-vision, servername: $DEST_DOMAIN, reality-opts: {public-key: $PUBLIC_KEY, short-id: $SHORT_ID}, client-fingerprint: firefox}"
echo ""
echo ""
echo "- 免流VLESS80-${MY_SUB}"
echo "- 歇斯底里${MY_SUB}"
echo "- Reality-XTLS-Vision443-${MY_SUB}"
echo ""
echo "=================================================================="
echo "               ✨ ✨ 【2】通用分享链接 ✨ ✨ "
echo ""
echo "vless://$MY_GUID@$SUB_DOMAIN:80?encryption=none&security=none&type=ws&host=$ML_HOST&path=/videos#免流VLESS80-${MY_SUB}"
echo "hysteria2://$MY_GUID@$SUB_DOMAIN:443/?obfs=salamander&obfs-password=$HY2_OBFS&sni=$SUB_DOMAIN#歇斯底里${MY_SUB}"
echo "vless://$MY_GUID@$VX_DOMAIN:443?encryption=none&security=tls&sni=$VX_DOMAIN&type=xhttp&path=/videos#XHTTP-CDN${PORT_CDN_VX}-${MY_SUB}"
echo "vless://$MY_GUID@${MY_SUB}vx${PORT_CDN_VX}.cf.cname.vvhan.com:443?encryption=none&security=tls&sni=$VX_DOMAIN&type=xhttp&path=/videos#XHTTP-CDN${PORT_CDN_VX}-${MY_SUB}优选域名"
echo "vless://$MY_GUID@$SUB_DOMAIN:${PORT_XHTTP_REALITY}?security=reality&pbk=$PUBLIC_KEY&sid=$SHORT_ID&fp=firefox&type=xhttp&path=/videos&sni=$DEST_DOMAIN#XHTTP-Reality${PORT_XHTTP_REALITY}-${MY_SUB}"
echo "vless://$MY_GUID@$SUB_DOMAIN:443?security=reality&pbk=$PUBLIC_KEY&sid=$SHORT_ID&fp=firefox&type=tcp&flow=xtls-rprx-vision&sni=$DEST_DOMAIN#Reality-XTLS-Vision443-${MY_SUB}"
echo ""
echo "✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ ✅ "
ULTIMATE_EOF
