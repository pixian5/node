echo "免流原理：已成功的方法之一是搭建如vless，端口选80，传输层协议选httpupgrade，host填能免流的域名，如v9-y.douyinvod.com"
...
cat > "$SBOX_CONF" << CONFIG
{
  ...
  "inbounds": [
    ...
    { "type": "vless", "tag": "vless-httpupgrade-ml", "listen": "::", "listen_port": 80, "users": [{ "uuid": "$MY_GUID" }], "transport": { "type": "httpupgrade", "path": "/videos", "host": "$ML_HOST" } }
  ],
...
echo ""
#Clash Party （Mihomo）不支持http-upgrade、xhttp传输方式
#echo "- {name: \"免流VLESS-${MY_SUB}\", type: vless, server: $SUB_DOMAIN, port: 80, uuid: $MY_GUID, network: http-upgrade, http-upgrade-opts: {path: /videos, headers: {Host: $ML_HOST}}}"
...
echo "               ✨ ✨ 【2】通用分享链接 ✨ ✨ "
echo ""
echo "vless://$MY_GUID@$SUB_DOMAIN:80?encryption=none&security=none&type=httpupgrade&host=$ML_HOST&path=/videos#免流VLESS-${MY_SUB}"
